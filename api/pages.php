 ...
 *   GET  ?action=reset_defaults     → Re-seed from setup_db.php (redirect)
 */
...
function ensureSitePagesTable(PDO $db): void {
    $db->exec("
...
// ─────────────────────────────────────────────────────────────
// GET reset_defaults → redirect to setup_db.php
// ─────────────────────────────────────────────────────────────
if ($method === 'GET' && $action === 'reset_defaults') {
    // Wipe site_pages and re-run setup
    try {
        $db->exec("DELETE FROM `site_pages`");
    } catch (Throwable $e) {}
    header('Location: /api/setup_db.php');
    exit();
}
...
respond([
    ...
    'endpoints' => [
        ...
        'GET ?action=reset_defaults'    => 'Wipe site_pages and re-seed from setup_db.php',
    ],
], 400);
